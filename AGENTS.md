...
- 진실 공식은 `metrics.py`의 `compute_zcs()`. SQL에서도 동일 공식(`MAX(0, 1.1*obj_time + 8*capture_kill + 4.1*(kills - capture_kill) - 5*deaths)`)을 쓰며, `_adapt_sql`이 Postgres용으로 `GREATEST(0, ...)`로 변환한다.
...
- **AP%** = `(캡처킬 / 킬) × 100` — **킬 중 거점 안 킬 비율**. "캡처킬" 필드는 거점(안)에서 기록한 킬 수로 **킬 컬럼의 부분집합**(게임이 캡처킬을 일반 킬에 중복 집계 — OCR 원본값, 배포 DB 전수 확인 결과 CK>킬 행 0건). 높을수록 킬이 거점 중심.
