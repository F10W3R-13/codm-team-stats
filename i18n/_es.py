# es 문자열 사전 — i18n/_ko.py, _en.py, _es.py 중 하나.
# 키 추가/수정 시 다른 언어 파일도 동기화 필수 (test_i18n.py가 검증).

STRINGS = {
"nav_brand": "🎯 CODM Stats",
        "nav_hub": "Centro",
        "nav_players": "Jugadores",
        "nav_leaderboard": "Ranking",
        "nav_compare": "Comparar",
        "nav_matches": "Partidas",
        "nav_timeseries": "Tendencias",
        "lang_label": "Idioma",
        "theme_toggle": "Cambiar tema",
        "season_label": "Vista de temporada",
        "season_s1": "Temporada 1 (archivo)",
        "season_s2": "Temporada 2 (actual)",

        "mode_hp": "Punto Fuerte",
        "mode_snd": "Buscar y Destruir",
        "all": "Todos",
        "date": "Fecha",
        "map": "Mapa",
        "player": "Jugador",
        "players": "Jugadores",
        "matches": "Partidas",
        "match": "Partida",
        "no_data": "Sin datos",
        "map_unknown": "Mapa desconocido",

        "period": "Periodo",
        "total_matches": "Partidas Totales",
        "stat_players": "Jugadores",
        "recent_matches": "🎮 Partidas Recientes",
        "mode_col": "Modo",

        "players_title": "👥 Estadísticas de Jugadores",
        "matches_col": "Partidas",
        "avg_k": "K Prom",
        "avg_d": "D Prom",
        "kd": "K/D",
        "avg_obj": "OBJ Prom",
        "avg_a": "A Prom",
        "avg_adr": "ADR Prom",
        "avg_score": "Score Prom",
        "avg_impact": "Impact Prom",
        "custom_metrics_desc": "DPD = Daño/Muertes · DPK = Daño/Bajas · ID = Impact − Score/34 · AP% = (CapKill/Bajas)×100 · ZCS = max(0, 1.1·OBJ+8·CK+4.1·(K−CK)−5·D)",

        "hp_section": "🎯 Punto Fuerte",
        "snd_section": "🔍 Buscar y Destruir",
        "games": "partidas",
        "avg_kd": "K / D Prom",
        "obj_seconds": "OBJ Prom",
        "avg_total_dmg": "Daño Total Prom",
        "avg_cap_kill": "Capture Kill Prom",
        "avg_kda": "K / D / A Prom",
        "avg_fk": "First Kill Prom",
        "avg_lww": "Lone Wolf Win Prom",
        "hp_trend_title": "📈 Tendencia K/D HP (últimas 30 partidas)",
        "snd_trend_title": "📈 Tendencia K/D SND (últimas 30 partidas)",
        "consistency_title": "Consistencia de Forma",
        "consistency_help": "σ (desviación) baja = más constante. Alta = más irregular.",
        "vs_team_title": "vs Promedio del Equipo",
        "custom_metrics_title": "🧮 Métricas Avanzadas",
        "lower_better": "menor es mejor",
        "insight_title": "Análisis",

        "leaderboard_title": "🏆 Ranking",
        "basis": "Métrica",
        "rank": "#",
        "lb_ranking": "Clasificación",
        "lb_vs_avg": "vs Promedio",
        "lb_team_avg": "Promedio",
        "player_maps_title": "🗺️ Rendimiento por Mapa",
        "player_maps_help": "vs promedio personal",
        "player_maps_help_snd": "RDS por mapa SND (vs propio promedio)",
        "player_maps_no_data_hp": "Datos insuficientes de mapas HP",
        "player_maps_no_data_snd": "Datos insuficientes de mapas SND",
        "player_maps_strong": "Fuerte",
        "player_maps_weak": "Débil",
        "player_maps_avg": "Promedio personal",

        "matches_title": "🎮 Historial de Partidas",
        "total_count": "Total",
        "avg_kd_col": "K/D Prom",
        "back_to_list": "← Volver a la lista",
        "mom_title": "👑 Jugador del Partido",
        "team_avg": "📊 Promedio del Equipo",
        "player_stats_title": "📋 Estadísticas de Jugadores",
        "kills": "Bajas",
        "deaths": "Muertes",
        "assists": "Asistencias",
        "score": "Score",
        "dmg": "Daño",
        "obj": "OBJ",
        "cap": "Cap",
        "fk": "FK",
        "lww": "LWW",
        "prev": "← Anterior",
        "next": "Siguiente →",

        "timeseries_title": "📈 Análisis de Tendencias",
        "select_player": "Seleccionar jugador",
        "select_metric": "Seleccionar métrica",
        "timeseries_help": "Elige un jugador y métrica para ver la tendencia por partida.",

        # Team insights
        "nav_insights": "Análisis del Equipo",
        "team_trend_title": "Tendencia del Equipo (Reciente vs Temporada)",
        "recent_period": "Reciente",
        "season": "Temporada",
        "trend_days": "Periodo de tendencia",
        "days_suffix": "d",
        "map_team_title": "Rendimiento por Mapa",
        "map_ace": "As del Mapa",
        "map_weak": "Punto Débil del Mapa",
        "map_detail_expand": "Expandir detalles de jugadores por mapa",

        "m_kills": "Bajas",
        "m_deaths": "Muertes",
        "m_kd": "K/D",
        "m_obj": "OBJ (seg)",
        "m_score": "Score",
        "m_impact": "Impact",
        "m_dmg": "Daño Total",
        "m_cap": "Capture Kill",
        "m_dpd": "DPD (Daño/Muerte)",
        "m_dpk": "DPK (Daño/Baja)",
        "m_id": "ID (Delta Impact)",
        "m_ap_pct": "AP% (Ratio Cap)",
        "m_zcs": "ZCS (Control de Zona)",
        "m_rds": "RDS (Dominio de Ronda)",

        "analysis_done": "✅ **{mode}** análisis completo — {n} jugadores guardados (partida #{id})",
        "date_label": "Fecha",
        "auto_report_failed": "(generación de informe falló — registro guardado)",

        # Admin
        "nav_admin": "Admin",
        "admin_login_title": "Acceso de Entrenador",
        "admin_login_help": "Contraseña para acceder a la pestaña Admin",
        "admin_password": "Contraseña",
        "admin_login_btn": "Entrar",
        "admin_title": "🛠️ Gestión de Datos",
        "admin_help": "Haz clic en una partida para editar meta/stats o eliminarla.",
        "filter_no_result": "Solo partidas sin resultado",
        "edit": "Editar",
        "delete": "Eliminar",
        "save": "Guardar",
        "cancel": "Cancelar",
        "team_score": "Nuestro puntaje",
        "opponent_score": "Puntaje rival",
        "result": "Resultado",
        "match_meta": "Info de Partida",
        "day_review_title": "Editar Revisión",
        "day_review_help": "Gestiona el VOD/nota del coach/resumen de transcripción de todo el día. Se aplica a todas las partidas de esta fecha.",
        "player_stats": "Stats de Jugadores",
        "vod_link": "VOD",
        "coach_note": "Nota del Coach",
        "coach_note_placeholder": "Revisión cualitativa, puntos de repaso, correcciones para el próximo juego...",
        "transcript_title": "Subir Transcripción",
        "transcript_help": "Sube una transcripción (.txt/.md) y la IA genera un resumen de coach. Solo se guarda el resumen — el original no se almacena.",
        "transcript_current": "Resumen guardado actual",
        "transcript_upload": "Subir y Resumir",
        "transcript_summary": "Resumen de Transcripción",
        "transcript_processing": "IA resumiendo...",
        "transcript_no_file": "Selecciona un archivo.",
        "transcript_done": "Resumen guardado",
        "transcript_err_large": "Archivo demasiado grande (máx 1.5MB).",
        "transcript_err_decode": "Error de decodificación (solo UTF-8).",
        "transcript_err_empty": "Archivo vacío.",
        "transcript_err_ai": "Resumen de IA falló. Intenta de nuevo.",
        "date_unknown": "Fecha desconocida",
        "confirm_delete": "¿Eliminar esta partida? No se puede deshacer.",
        "saved_ok": "Guardado",
        "saving": "Guardando...",
        "deleted_ok": "Eliminado",
        "add_player": "Añadir Jugador",
        "added_ok": "Jugador añadido",
        "stat_id": "ID",
        # Alias
        "nav_admin_aliases": "Aliases",
        "admin_tab_matches": "Partidas",
        "admin_tab_aliases": "Aliases",
        "admin_tab_players": "Jugadores",
        "admin_tab_opponents": "Rivales",
        "opp_title": "Equipos rivales",
        "opp_add_team": "Añadir equipo",
        "opp_team_name": "Nombre del equipo",
        "opp_add": "Añadir",
        "opp_roster_paste": "Pegar roster (un apodo por línea)",
        "opp_roster_add": "Añadir roster",
        "opp_player": "Jugador",
        "opp_source": "Origen",
        "opp_matches": "Partidos",
        "opp_pending_title": "Partidos con rival sin identificar",
        "opp_date": "Fecha", "opp_mode": "Modo", "opp_map": "Mapa", "opp_result": "Resultado",
        "opp_score": "Marcador",
        "opp_assign_team": "Asignar equipo",
        "opp_save": "Guardar",
        "opp_saved": "Guardado",
        "opp_error": "Falló",
        "opp_rename": "Renombrar",
        "opp_rename_save": "Guardar",
        "opp_delete_team": "Eliminar equipo",
        "opp_confirm_delete_team": "¿Eliminar este equipo? Se quitará la etiqueta de {n} partido(s) y la plantilla (jugadores y estadísticas se conservan).",
        "opp_remove_roster": "Quitar de la plantilla",
        "opp_confirm_remove_roster": "¿Quitar a {name} de la plantilla de este equipo?",
        "opp_unassign": "Desasignar",
        "opp_unassign_pick": "Elige un partido para desasignar",
        "opp_ignore_alias": "Reasignar ignorando alias obsoletos",
        "opp_pending_more": "…{n} más (se muestran los últimos 50)",
        "opp_confirm_merge": "¿Fusionar con {dst}? No se puede deshacer.",
        "opp_alias_title": "Alias de jugadores rivales",
        "opp_alias_hint": "Vincula manualmente variantes de escritura OCR a un jugador. Al borrar, se reaprende en el siguiente guardado.",
        "opp_alias_ign": "Alias (IGN)",
        "opp_alias_target": "Jugador",
        "opp_alias_add": "Añadir alias",
        "opp_source_registered": "Manual",
        "opp_source_match": "Auto (partido)",
        "opp_source_manual": "Script",
        "opp_session_expired": "Sesión caducada — inicia sesión de nuevo",
        "opp_suggest_badge": "Probable {team} · {n} partidos de evidencia",
        "opp_suggest_apply": "Asignar",
        "opp_confirm_unassign": "¿Desasignar el equipo del partido #{n}?",
        "opp_merge_title": "Fusionar jugadores rivales",
        "opp_merge_hint": "Muestra solo jugadores sin equipo o con nombres OCR ilegibles. Si el mismo jugador está dividido bajo distintos nombres, fúndelos. Una fusión se aprende para siempre.",
        "opp_no_team": "Sin equipo",
        "opp_ocr_suspect": "OCR dudoso",
        "opp_teams_col": "Equipo",
        "opp_stat_rows": "Filas de stats",
        "opp_merge_into": "Fusionar en",
        "opp_assign_player": "Jugador destino",
        "opp_merge": "Fusionar",
        "nav_versus": "Versus",
        "versus_title": "Versus",
        "versus_no_data": "Aún no hay datos de rivales",
        "versus_record": "Historial",
        "versus_matches": "Partidos",
        "versus_h2h": "Enfrentamientos jugador a jugador",
        "versus_our_player": "Nuestro jugador",
        "versus_opp_player": "Jugador rival",
        "versus_matches_together": "Partidos juntos",
        "versus_kd_diff": "Dif. K-D",
        "versus_metric_diff": "Dif. ZCS/RDS",
        "versus_match_history": "Historial de partidos",
        "versus_pending_hint": "Los partidos sin asignar quedan excluidos",
        "versus_avg_margin": "Margen medio",
        "versus_no_result": "Sin registrar",
        "versus_back": "Volver a la lista",
        "versus_win_rate": "Tasa de victoria",
        "versus_games": "Partidos",
        "versus_empty_hint": "Aún no hay partidos asignados a este equipo. Asigna uno en el panel de administración.",
        "players_admin_help": "Jugadores registrados. Elimina jugadores mal creados (p.ej. Swish) o fúndelos con otro jugador (transfiere stats + alias). Los IGN de invitados/fallos de OCR también se pueden fusionar aquí con jugadores oficiales.",
        "players_admin_warning": "Eliminar/fusionar es irreversible. Se recomienda respaldo. Eliminar un jugador borra todos sus stats y alias.",
        "merge_dst": "Fusionar en",
        "merge": "Fusionar",
        "merge_select_dst": "Selecciona un jugador destino.",
        "confirm_merge": "Transferir todos los stats + alias a {dst} y eliminar este jugador. ¿Continuar?",
        "merged_ok": "Fusionado",
        "confirm_delete_player": "Eliminar permanentemente a {name} y todos sus stats + alias. ¿Continuar?",
        "alias_title": "🏷️ Gestión de Alias",
        "alias_subtitle": "Variante de IGN → jugador. 🤖 marca alias aprendidos automáticamente — revísalos periódicamente.",
        "alias_add": "Añadir alias",
        "alias_add_btn": "Añadir",
        "alias_ign": "IGN (nombre en pantalla)",
        "alias_select_player": "Seleccionar jugador",
        "alias_list": "Aliases",
        "alias_source": "Origen",
        "source_manual": "Manual",
        "source_ocr_auto": "OCR Auto",
        "source_merge": "Fusionado",
        "alias_empty": "No hay alias registrados.",
        "alias_fill_both": "Introduce IGN y jugador.",
        "alias_confirm_delete": "¿Eliminar este alias?",
        "win": "V",
        "loss": "D",
        "draw": "E",
        "result_none": "N/A",
        "result_col": "Resultado",
        "score_col": "Marcador",
        "win_rate": "Tasa de Victorias",
        "no_result_data": "Sin resultados registrados. Regístralos en /admin.",
        "match_insight_title": "🧠 Análisis IA",
        "insight_loading": "Análisis no disponible (revisa la API key)",
        "insight_fetching": "Generando análisis IA",
        "hub_title": "Centro de Coaching",
        "hub_subtitle": "Lo que hay que ver ahora — de un vistazo",
        "hub_missing_result": "Partidas sin resultado",
        "hub_missing_result_help": "No se refleja en win rate / tendencias hasta introducirlo",
        "hub_missing_result_go": "Introducir resultados",
        "hub_team_trend": "Tendencia del Equipo",
        "hub_kd": "K/D",
        "hub_kills": "Bajas",
        "hub_dmg": "Daño",
        "hub_season": "Temporada",
        "hub_recent": "Reciente",
        "hub_form_alerts": "Alertas de Forma",
        "hub_form_help": "Jugadores con K/D reciente ≥10% por debajo del promedio",
        "hub_form_up": "Subida de Forma",
        "hub_form_up_help": "Jugadores con K/D reciente ≥10% por encima del promedio",
        "hub_season_kd": "K/D Temporada",
        "hub_recent_kd": "K/D Reciente",
        # 기간 토글
        "hub_period_5": "Últimas 5",
        "hub_period_10": "Últimas 10",
        "hub_period_season": "Temporada",
        # 밴픽보드
        "hub_banpick": "Ban/Pick",
        "hub_banpick_help": "Por mapa: periodo vs temporada — mapas al alza para pick, a la baja para ban",
        "hub_pick": "PICK",
        "hub_ban": "BAN",
        "hub_low_sample": "muestra baja",
        # 역할 스펙트럼
        "hub_role_spectrum": "Espectro de Roles",
        "hub_spectrum_help": "Posición en eje slay ↔ obj. Desviaciones ampliadas para legibilidad.",
        "hub_spectrum_slay": "SLAY",
        "hub_spectrum_obj": "OBJ",
        "hub_spectrum_balanced": "EQUILIBRADO",
        # 코칭 노트 (액션 아이템)
        "notes_title": "Tareas",
        "notes_add": "Añadir",
        "notes_add_placeholder": "Corregir antes del próximo partido…",
        "notes_match_placeholder": "Encontrado en este partido…",
        "notes_player_none": "Sin jugador",
        "notes_resolve": "Marcar como resuelto",
        "notes_reopen": "Reabrir",
        "notes_elapsed": "transcurridos",
        "notes_empty": "No hay tareas abiertas 🎉",
        "notes_match_empty": "Sin tareas para este partido.",
        # Briefing previo al partido (solo entrenador)
        "briefing_title": "Briefing Pre-partido",
        "briefing_generate": "Generar Briefing",
        "briefing_loading": "Generando…",
        "briefing_retry": "Reintentar",
        "matches_suffix": "partidos",
        "hub_link_insights": "Análisis IA · stats por mapa",
        "hub_link_players": "Stats de jugadores · métricas",
        "zcs_label": "ZCS",
        "zcs_full": "Puntaje Control de Zona",
        "zcs_team": "ZCS Promedio del Equipo",
        "zcs_trend_title": "Tendencia ZCS del Equipo",
        "avg_zcs_col": "ZCS Promedio",
        "rds_label": "RDS",
        "rds_full": "Puntuación de Dominio de Ronda",
        "rds_team": "RDS Promedio del Equipo",
        "avg_rds_col": "RDS Promedio",
        "compare_title": "Comparar Jugadores",
        "compare_help": "Selecciona dos jugadores para comparar stats",
        "compare_player_a": "Jugador A",
        "compare_player_b": "Jugador B",
        "compare_select_two": "Selecciona dos jugadores",
        "compare_same": "Mismo jugador. Elige otro.",
        "compare_not_found": "Jugador no encontrado.",
        "compare_radar_title": "Radar de Habilidad",
        "compare_detail": "Detalle",
        "metric_label": "Métrica",
        "role_slayer": "Slayer",
        "role_objective": "Objetivo",
        "role_balanced": "Equilibrado",
        "role_title": "Distribución de Roles",
        "role_help": "Clasificación automática por stats HP (vs promedio)",
        "role_badge": "Rol",
        "nav_maps": "Mapas",
        "maps_title": "🗺️ Stats de Mapas",
        "map_detail_title": "Detalle de Mapa",
        "map_win_rate": "TV del Mapa",
        "map_vs_team": "vs Promedio",
        "map_recent_vs_season": "Reciente vs Temporada",
        "map_advice_title": "📊 Tendencias Numéricas",
        "map_no_data": "Sin datos para este mapa",
        "map_matches_col": "Partidas",
}
