# en 문자열 사전 — i18n/_ko.py, _en.py, _es.py 중 하나.
# 키 추가/수정 시 다른 언어 파일도 동기화 필수 (test_i18n.py가 검증).

STRINGS = {
"nav_brand": "🎯 CODM Stats",
        "nav_hub": "Hub",
        "nav_players": "Players",
        "nav_leaderboard": "Leaderboard",
        "nav_compare": "Compare",
        "nav_matches": "Matches",
        "nav_timeseries": "Trends",
        "lang_label": "Language",
        "theme_toggle": "Toggle theme",
        "season_label": "Season view",
        "season_s1": "Season 1 (archive)",
        "season_s2": "Season 2 (current)",

        "mode_hp": "Hardpoint",
        "mode_snd": "Search & Destroy",
        "all": "All",
        "date": "Date",
        "map": "Map",
        "player": "Player",
        "players": "Players",
        "matches": "Matches",
        "match": "Match",
        "no_data": "No data",
        "map_unknown": "Unknown map",

        "period": "Period",
        "total_matches": "Total Matches",
        "stat_players": "Players",
        "recent_matches": "🎮 Recent Matches",
        "mode_col": "Mode",

        "players_title": "👥 Player Stats",
        "matches_col": "Matches",
        "avg_k": "Avg K",
        "avg_d": "Avg D",
        "kd": "K/D",
        "avg_obj": "Avg OBJ",
        "avg_a": "Avg A",
        "avg_adr": "Avg ADR",
        "avg_score": "Avg Score",
        "avg_impact": "Avg Impact",
        "custom_metrics_desc": "DPD = DMG/Deaths · DPK = DMG/Kills · ID = Impact − Score/34 · AP% = (CapKill/Kills)×100 · ZCS = max(0, 1.1·OBJ+8·CK+4.1·(K−CK)−5·D)",

        "hp_section": "🎯 Hardpoint",
        "snd_section": "🔍 Search & Destroy",
        "games": "matches",
        "avg_kd": "Avg K / D",
        "obj_seconds": "Avg OBJ",
        "avg_total_dmg": "Avg Total DMG",
        "avg_cap_kill": "Avg Capture Kill",
        "avg_kda": "Avg K / D / A",
        "avg_fk": "Avg First Kill",
        "avg_lww": "Avg Lone Wolf Win",
        "hp_trend_title": "📈 HP K/D Trend (last 30 matches)",
        "snd_trend_title": "📈 SND K/D Trend (last 30 matches)",
        "consistency_title": "Form Consistency",
        "consistency_help": "Lower σ (std dev) = more consistent. Higher = more volatile.",
        "vs_team_title": "vs Team Average",
        "custom_metrics_title": "🧮 Advanced Metrics",
        "lower_better": "lower is better",
        "insight_title": "Insight",

        "leaderboard_title": "🏆 Leaderboard",
        "basis": "Metric",
        "rank": "#",
        "lb_ranking": "Ranking",
        "lb_vs_avg": "vs Team Avg",
        "lb_team_avg": "Team Avg",
        "player_maps_title": "🗺️ Map Performance",
        "player_maps_help": "vs personal avg",
        "player_maps_help_snd": "This player's RDS by SND map (vs own average)",
        "player_maps_no_data_hp": "Not enough HP map data for this player",
        "player_maps_no_data_snd": "Not enough SND map data for this player",
        "player_maps_strong": "Strong",
        "player_maps_weak": "Weak",
        "player_maps_avg": "Personal avg",

        "matches_title": "🎮 Match History",
        "total_count": "Total",
        "avg_kd_col": "Avg K/D",
        "back_to_list": "← Back to list",
        "mom_title": "👑 Man of the Match",
        "team_avg": "📊 Team Average",
        "player_stats_title": "📋 Player Stats",
        "kills": "Kills",
        "deaths": "Deaths",
        "assists": "Assists",
        "score": "Score",
        "dmg": "DMG",
        "obj": "OBJ",
        "cap": "Cap",
        "fk": "FK",
        "lww": "LWW",
        "prev": "← Prev",
        "next": "Next →",

        "timeseries_title": "📈 Trend Analysis",
        "select_player": "Select player",
        "select_metric": "Select metric",
        "timeseries_help": "Pick a player and metric to see per-match trends.",

        # Team insights
        "nav_insights": "Team Insights",
        "team_trend_title": "Team Trend (Recent vs Season)",
        "recent_period": "Recent",
        "season": "Season",
        "trend_days": "Trend period",
        "days_suffix": "d",
        "map_team_title": "Performance by Map",
        "map_ace": "Map Ace",
        "map_weak": "Map Weak",
        "map_detail_expand": "Expand per-map player details",

        "m_kills": "Kills",
        "m_deaths": "Deaths",
        "m_kd": "K/D",
        "m_obj": "OBJ (sec)",
        "m_score": "Score",
        "m_impact": "Impact",
        "m_dmg": "Total DMG",
        "m_cap": "Capture Kill",
        "m_dpd": "DPD (DMG/Death)",
        "m_dpk": "DPK (DMG/Kill)",
        "m_id": "ID (Impact Delta)",
        "m_ap_pct": "AP% (Cap Ratio)",
        "m_zcs": "ZCS (Zone Control)",
        "m_rds": "RDS (Round Domination)",

        "analysis_done": "✅ **{mode}** analysis done — {n} players saved (match #{id})",
        "date_label": "Date",
        "auto_report_failed": "(report generation failed — record saved)",

        # Admin
        "nav_admin": "Admin",
        "admin_login_title": "Coach Login",
        "admin_login_help": "Password to access the Admin tab",
        "admin_password": "Password",
        "admin_login_btn": "Log in",
        "admin_title": "🛠️ Data Management",
        "admin_help": "Click a match to edit its meta/stats or delete it.",
        "filter_no_result": "Only no-result matches",
        "edit": "Edit",
        "delete": "Delete",
        "save": "Save",
        "cancel": "Cancel",
        "team_score": "Our score",
        "opponent_score": "Opponent score",
        "result": "Result",
        "match_meta": "Match Info",
        "day_review_title": "Review Edit",
        "day_review_help": "Manage the VOD/coach note/transcript summary for this entire day. Applies to all matches on this date.",
        "player_stats": "Player Stats",
        "vod_link": "VOD",
        "coach_note": "Coach Note",
        "coach_note_placeholder": "Qualitative review, replay points, corrections for next game...",
        "transcript_title": "Transcript Upload",
        "transcript_help": "Upload a match transcript (.txt/.md) and AI generates a coaching summary. Only the summary is saved — the original is not stored.",
        "transcript_current": "Current saved summary",
        "transcript_upload": "Upload & Summarize",
        "transcript_summary": "Transcript Summary",
        "transcript_processing": "AI summarizing...",
        "transcript_no_file": "Please select a file.",
        "transcript_done": "Summary saved",
        "transcript_err_large": "File too large (max 1.5MB).",
        "transcript_err_decode": "Decode failed (UTF-8 text only).",
        "transcript_err_empty": "File is empty.",
        "transcript_err_ai": "AI summary failed. Try again.",
        "date_unknown": "Unknown date",
        "confirm_delete": "Delete this match? This cannot be undone.",
        "saved_ok": "Saved",
        "saving": "Saving...",
        "deleted_ok": "Deleted",
        "add_player": "Add Player",
        "added_ok": "Player added",
        "stat_id": "ID",
        # Alias 관리
        "nav_admin_aliases": "Aliases",
        "admin_tab_matches": "Matches",
        "admin_tab_aliases": "Aliases",
        "admin_tab_players": "Players",
        "admin_tab_opponents": "Opponents",
        "opp_title": "Opponent Teams",
        "opp_add_team": "Add Team",
        "opp_team_name": "Team name",
        "opp_add": "Add",
        "opp_roster_paste": "Paste roster (one nickname per line)",
        "opp_roster_add": "Add roster",
        "opp_player": "Player",
        "opp_source": "Source",
        "opp_matches": "Matches",
        "opp_pending_title": "Matches with unidentified opponent",
        "opp_date": "Date", "opp_mode": "Mode", "opp_map": "Map", "opp_result": "Result",
        "opp_score": "Score",
        "opp_assign_team": "Assign team",
        "opp_save": "Save",
        "opp_saved": "Saved",
        "opp_error": "Failed",
        "opp_rename": "Rename",
        "opp_rename_save": "Save",
        "opp_delete_team": "Delete team",
        "opp_confirm_delete_team": "Delete this team? {n} match tag(s) will be cleared and the roster removed (players & stats are kept).",
        "opp_remove_roster": "Remove from roster",
        "opp_confirm_remove_roster": "Remove {name} from this team's roster?",
        "opp_unassign": "Unassign",
        "opp_unassign_pick": "Pick a match to unassign",
        "opp_ignore_alias": "Re-match ignoring stale aliases",
        "opp_pending_more": "…{n} more (showing latest 50)",
        "opp_confirm_merge": "Merge into {dst}? This cannot be undone.",
        "opp_alias_title": "Opponent player aliases",
        "opp_alias_hint": "Manually link OCR spelling variants to a player. Deleting allows re-learning on the next save.",
        "opp_alias_ign": "Alias (IGN)",
        "opp_alias_target": "Player",
        "opp_alias_add": "Add alias",
        "opp_source_registered": "Manual",
        "opp_source_match": "Auto (match)",
        "opp_source_manual": "Script",
        "opp_session_expired": "Session expired — please log in again",
        "opp_confirm_unassign": "Unassign team from match #{n}?",
        "opp_merge_title": "Merge opponent players",
        "opp_merge_hint": "Shows only players with no team or garbled OCR names. If the same player is split under different names, merge them. One merge is learned permanently.",
        "opp_no_team": "No team",
        "opp_ocr_suspect": "OCR suspect",
        "opp_teams_col": "Team",
        "opp_stat_rows": "Stat rows",
        "opp_merge_into": "Merge into",
        "opp_assign_player": "Target player",
        "opp_merge": "Merge",
        "nav_versus": "Versus",
        "versus_title": "Versus",
        "versus_no_data": "No opponent data yet",
        "versus_record": "Record",
        "versus_matches": "Matches",
        "versus_h2h": "Player head-to-head",
        "versus_our_player": "Our player",
        "versus_opp_player": "Opponent player",
        "versus_matches_together": "Shared matches",
        "versus_kd_diff": "K-D diff",
        "versus_metric_diff": "ZCS/RDS diff",
        "versus_match_history": "Match history",
        "versus_pending_hint": "Unassigned matches are excluded",
        "versus_avg_margin": "Avg margin",
        "versus_no_result": "Not entered",
        "versus_back": "Back to versus list",
        "versus_win_rate": "Win rate",
        "versus_games": "Games",
        "versus_empty_hint": "No matches assigned to this team yet. Assign one in the admin page.",
        "players_admin_help": "Registered players. Delete mis-created players (e.g. Swish) or merge into another player (transfers stats + aliases). Guest/OCR-failed IGNs can be merged into official players here too.",
        "players_admin_warning": "Delete/merge is irreversible. Backup recommended. Deleting a player removes all their stats and aliases.",
        "merge_dst": "Merge into",
        "merge": "Merge",
        "merge_select_dst": "Select a target player to merge into.",
        "confirm_merge": "Transfer all stats + aliases to {dst} and delete this player. Continue?",
        "merged_ok": "Merged",
        "confirm_delete_player": "Permanently delete {name} and all their stats + aliases. Continue?",
        "alias_title": "🏷️ Alias Management",
        "alias_subtitle": "IGN variant → player mapping. 🤖 marks auto-learned OCR aliases — review and clean them periodically.",
        "alias_add": "Add Alias",
        "alias_add_btn": "Add",
        "alias_ign": "IGN (name shown on screen)",
        "alias_select_player": "Select player",
        "alias_list": "Aliases",
        "alias_source": "Source",
        "source_manual": "Manual",
        "source_ocr_auto": "OCR Auto",
        "source_merge": "Merged",
        "alias_empty": "No aliases registered.",
        "alias_fill_both": "Enter both IGN and player.",
        "alias_confirm_delete": "Delete this alias?",
        "win": "W",
        "loss": "L",
        "draw": "D",
        "result_none": "N/A",
        "result_col": "Result",
        "score_col": "Score",
        "win_rate": "Win Rate",
        "no_result_data": "No results entered yet. Register outcomes in /admin.",
        "match_insight_title": "🧠 AI Match Analysis",
        "insight_loading": "Analysis unavailable (check API key)",
        "insight_fetching": "Generating AI analysis",
        "hub_title": "Coaching Hub",
        "hub_subtitle": "What to look at now — at a glance",
        "hub_missing_result": "Matches missing a result",
        "hub_missing_result_help": "Not reflected in win rate / form charts until entered",
        "hub_missing_result_go": "Enter results",
        "hub_team_trend": "Team Trend",
        "hub_kd": "K/D",
        "hub_kills": "Kills",
        "hub_dmg": "Damage",
        "hub_season": "Season",
        "hub_recent": "Recent",
        "hub_form_alerts": "Form Alerts",
        "hub_form_help": "Players with recent K/D down ≥10% vs season avg",
        "hub_form_up": "Form Up",
        "hub_form_up_help": "Players with recent K/D up ≥10% vs season avg",
        "hub_season_kd": "Season K/D",
        "hub_recent_kd": "Recent K/D",
        # 기간 토글
        "hub_period_5": "Last 5",
        "hub_period_10": "Last 10",
        "hub_period_season": "Season",
        # 밴픽보드
        "hub_banpick": "Ban/Pick Board",
        "hub_banpick_help": "Per map: period vs season — rising maps to pick, falling to ban",
        "hub_pick": "PICK",
        "hub_ban": "BAN",
        "hub_low_sample": "low sample",
        # 역할 스펙트럼
        "hub_role_spectrum": "Role Spectrum",
        "hub_spectrum_help": "Position on slay ↔ obj axis. Deviations are magnified for readability.",
        "hub_spectrum_slay": "SLAY",
        "hub_spectrum_obj": "OBJ",
        "hub_spectrum_balanced": "BALANCED",
        # 코칭 노트 (액션 아이템)
        "notes_title": "Action Items",
        "notes_add": "Add",
        "notes_add_placeholder": "Fix before next match…",
        "notes_match_placeholder": "Found in this match…",
        "notes_player_none": "No player tag",
        "notes_resolve": "Mark as resolved",
        "notes_reopen": "Reopen",
        "notes_elapsed": "elapsed",
        "notes_empty": "No open action items 🎉",
        "notes_match_empty": "No action items for this match.",
        # AI pre-match briefing (coach only)
        "briefing_title": "Pre-match Briefing",
        "briefing_generate": "Generate Briefing",
        "briefing_loading": "Generating…",
        "briefing_retry": "Retry",
        "matches_suffix": "matches",
        "hub_link_insights": "AI team analysis · per-map stats",
        "hub_link_players": "Player stats · custom metrics",
        "zcs_label": "ZCS",
        "zcs_full": "Zone Control Score",
        "zcs_team": "Team Avg ZCS",
        "zcs_trend_title": "Team ZCS Trend",
        "avg_zcs_col": "Avg ZCS",
        "rds_label": "RDS",
        "rds_full": "Round Domination Score",
        "rds_team": "Team Avg RDS",
        "avg_rds_col": "Avg RDS",
        "compare_title": "Player Compare",
        "compare_help": "Select two players to compare mode stats side by side",
        "compare_player_a": "Player A",
        "compare_player_b": "Player B",
        "compare_select_two": "Select two players",
        "compare_same": "Same player selected. Choose a different one.",
        "compare_not_found": "Player not found.",
        "compare_radar_title": "Ability Radar",
        "compare_detail": "Detail",
        "metric_label": "Metric",
        "role_slayer": "Slayer",
        "role_objective": "Objective",
        "role_balanced": "Balanced",
        "role_title": "Team Role Distribution",
        "role_help": "Auto role classification from HP stats (vs team avg)",
        "role_badge": "Role",
        "nav_maps": "Maps",
        "maps_title": "🗺️ Map Stats",
        "map_detail_title": "Map Detail",
        "map_win_rate": "Map Win Rate",
        "map_vs_team": "vs Team Avg",
        "map_recent_vs_season": "Recent vs Season",
        "map_advice_title": "📊 Numeric Trends",
        "map_no_data": "No data for this map",
        "map_matches_col": "Matches",
}
